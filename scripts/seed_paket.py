...
DUMMY_PAKETS = [
    {"nama": "Agus Santoso", "nomor_telepon": "081234567890",
     ...
     "cod": True, "harga": 250000, "catatan": "Hubungi sebelum antar"},
    {"nama": "Siti Rahayu", "nomor_telepon": "081398765432",
     ...
     "cod": False, "harga": 150000, "catatan": None},
    {"nama": "Andi Wijaya", "nomor_telepon": "082111223344",
     ...
     "cod": True, "harga": 500000, "catatan": "Barang fragile, jangan ditumpuk"},
    {"nama": "Dewi Lestari", "nomor_telepon": "085622334455",
     ...
     "cod": False, "harga": 75000, "catatan": None},
    {"nama": "Usman Bin Myra", "nomor_telepon": "087833445566",
     ...
     "cod": True, "harga": 1200000, "catatan": "COD tunai"},
]
    ...
    async with SessionLocal() as session:
        for item in DUMMY_PAKETS:
            exists = await session.execute(
                select(Paket.id).where(Paket.resi == item["resi"]))
            if exists.scalar() is not None:
                continue
            session.add(Paket(**item))
            inserted += 1
        await session.commit()
    print(f"Seed selesai: {inserted} baris baru, {len(DUMMY_PAKETS) - inserted} sudah ada.")
