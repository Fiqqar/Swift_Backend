...
2. **Pathfinding Endpoint Update (`app/api/v1/endpoints/pathfinding.py`):**
   ...
   - Pada `find_optimized_delivery_route` (Multi-stop solver): Urutkan urutan
     stop paling efisien memakai **hybrid greedy** (`optimize_stop_order_hybrid`,
     top-K kandidat haversine → jarak jalan nyata via cache Redis, EXPRESS
     diutamakan, dimulai dari posisi kurir webhook), simpan snapshot multi-leg
     ke Redis, dan kembalikan `route_id`.
